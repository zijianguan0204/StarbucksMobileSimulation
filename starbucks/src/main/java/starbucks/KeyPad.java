...
public class KeyPad implements ITouchEventHandler, IDisplayComponent, IKeyPadSubject
{
    ...
    /**
     * resetKeyPad() for reset the variable of keypad
     */
    ...
    public void touch(int x, int y) {
        if((x > 0 && x < 4))
        {
            if( y > 4 && y < 9)
            {
                ...
                notifyObservers() ;        //everytime when there is a key coming
            }
            else
            {
                if ( nextHandler != null )
    ...
    */
    ...
    public void notifyObservers( )
    {
        ...
        {
            IKeyPadObserver observer = observers.get(i) ; //scan all observer and do keyEventUpdate
            observer.keyEventUpdate( countPinDigits, lastKey ) ; // PinEntryMachine.java. get the total key number and the last key
        }
    }
